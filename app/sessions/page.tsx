        ...
        {sessions.map((s) => (
          <li key={s.id} className="flex items-center justify-between rounded border p-3">
            ...
            <div className="flex items-center gap-2">
              <button type="button" className="rounded border px-3 py-1 text-sm hover:bg-secondary" onClick={() => restoreSession(s.id)}>
                <RotateCcw className="mr-1 inline-block h-4 w-4"/> Restore
              </button>
              <button type="button" className="rounded border px-3 py-1 text-sm text-red-600 hover:bg-secondary" onClick={() => deleteSession(s.id)}>
