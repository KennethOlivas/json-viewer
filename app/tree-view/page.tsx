...
export default function TreeViewPage() {
  ...
  const setAtPath = (root: JSONValue, path: (string|number)[], value: JSONValue): JSONValue => {
  ...
  const onSave = () => { saveSession(); toast.success("Session saved"); };

  return (
    <div className="container mx-auto max-w-5xl px-4 py-6">
      <h1 className="mb-4 text-2xl font-semibold">Tree View</h1>
      <Toolbar>
        <ToolbarButton onClickAction={onSave} title="Save session"><Save className="h-4 w-4"/> Save</ToolbarButton>
      </Toolbar>
      <div className="mt-4 rounded border p-3">
        {data ? (
          <JsonTree value={data as JSONValue} onChangeAction={handleNodeChange} />
        ) : (
          <div className="text-muted-foreground">No JSON loaded. Use Raw View or Formatter to input JSON.</div>
        )}
      </div>

      <FAB onPressAction={() => window.scrollTo({ top: 0, behavior: 'smooth' })}>Top</FAB>
    </div>
  );
}
