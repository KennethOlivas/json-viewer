...
import { deleteAtPath, getAtPath, setAtPath, type JSONObject, type JSONArray, type JSONValue } from "@/lib/json";
...
export default function GraphViewPage() {
  ...
  const [editDlg, setEditDlg] = useState<{ open: boolean; node?: GraphNode; text: string }>({ open: false, text: "" });
  const [deleteDlg, setDeleteDlg] = useState<{ open: boolean; node?: GraphNode }>({ open: false });
  const [editKeyDlg, setEditKeyDlg] = useState<{ open: boolean; node?: GraphNode; keyText: string }>({ open: false, keyText: "" });
  const [addChildDlg, setAddChildDlg] = useState<{ open: boolean; node?: GraphNode; keyText: string; valueText: string }>({ open: false, keyText: "", valueText: "" });
  const [editValid, setEditValid] = useState<{ ok: boolean; error?: string }>({ ok: true });
  const [addValid, setAddValid] = useState<{ ok: boolean; error?: string }>({ ok: true });
  ...
  const onCopyPath = useCallback(() => {
    ...
    if (!node) return;
    const pathStr = node.path.map((p) => (typeof p === "number" ? `[${p}]` : `.${p}`)).join("") || "root";
  ...
  const exportNodePng = useCallback(() => {
    ...
    const url = graphApiRef.current.exportNodePng(node.id, { width: 240, height: 100, margin: 16 });
  ...
  const openEditValue = useCallback(() => {
    ...
    const text = typeof v === 'string' ? JSON.stringify(v) : JSON.stringify(v, null, 2);
  ...
  const openEditKey = useCallback(() => {
    const node = menu.node ?? mobileSheet.node;
    if (!node || !data) return;
    if (node.path.length === 0) { toast("Cannot rename root"); return; }
  ...
  const applyEditKey = useCallback(() => {
    const node = editKeyDlg.node; if (!node || !data) return;
    const parentPath = node.path.slice(0, -1);
    const last = node.path[node.path.length - 1];
    ...
    if (Object.prototype.hasOwnProperty.call(parent, newKey) && newKey !== last) { toast.error("Key already exists"); return; }
  ...
  const openAddChild = useCallback(() => {
    ...
    if (!isObj && !isArr) { toast("Add child is only available for objects/arrays"); return; }
  ...
  const applyAddChild = useCallback(() => {
    const node = addChildDlg.node; if (!node || !data) return;
    ...
    if (isObj) {
      ...
      if (!key) { toast.error("Key is required for objects"); return; }
      if ((cur as Record<string, unknown>)[key] !== undefined) { toast.error("Key already exists"); return; }
      const newObj: JSONObject = { ...(cur as JSONObject), [key]: val as JSONValue };
    ...
    setAddChildDlg({ open: false, node: undefined, keyText: "", valueText: "" });
  ...
  <div ref={containerRef} className="relative h-[calc(100dvh-56px)] w-full overflow-hidden bg-linear-to-b from-[#0d0d0d] to-[#1a1a1a]">
      ...
      {data && (
        domMode ? (
          <DOMGraph
            ...
            onNodeLongPressAction={(n) => onNodeLongPress(n as unknown as GraphNode)}
      ...
      <DropdownMenu open={menu.open} onOpenChange={(open) => setMenu((m) => ({ ...m, open }))}>
        ...
        <DropdownMenuContent side="bottom" align="start" sideOffset={6}>
          ...
          <DropdownMenuItem onClick={openEditValue}>Edit value</DropdownMenuItem>
          <DropdownMenuItem onClick={openAddChild}>Add child</DropdownMenuItem>
          ...
          <DropdownMenuItem onClick={() => setDeleteDlg({ open: true, node: menu.node })} className="text-destructive">Delete</DropdownMenuItem>
        </DropdownMenuContent>
      </DropdownMenu>

      {/* Mobile bottom sheet for long-press */}
      <Drawer open={mobileSheet.open} onOpenChange={(open) => setMobileSheet((s) => ({ ...s, open }))}>
        <DrawerContent>
          <div className="mx-auto w-full max-w-md p-4">
            <div className="mb-3 text-sm text-muted-foreground">{mobileSheet.node?.label}</div>
            <div className="grid gap-2">
              <Button variant="secondary" className="h-12 justify-start text-base" onClick={onCopyPath}>Copy path</Button>
              <Button variant="secondary" className="h-12 justify-start text-base" onClick={openEditKey}>Edit key</Button>
              <Button variant="secondary" className="h-12 justify-start text-base" onClick={openEditValue}>Edit value</Button>
              <Button variant="secondary" className="h-12 justify-start text-base" onClick={openAddChild}>Add child</Button>
              <Button variant="secondary" className="h-12 justify-start text-base" onClick={startLinkMode}>Link to node</Button>
              <Button variant="secondary" className="h-12 justify-start text-base" onClick={exportNodePng}>Export node PNG</Button>
              <Button variant="secondary" className="h-12 justify-start text-base" onClick={exportNodeSvg}>Export node SVG</Button>
              <Button variant="destructive" className="h-12 justify-start text-base" onClick={() => setDeleteDlg({ open: true, node: mobileSheet.node })}>Delete</Button>
            </div>
          </div>
        </DrawerContent>
      </Drawer>
      ...
      {/* Edit Value Dialog */}
      <Dialog open={editDlg.open} onOpenChange={(open) => setEditDlg((d) => ({ ...d, open }))}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Edit value</DialogTitle>
            <DialogDescription>Update the JSON value for this node. Live JSON validation below. For strings, include quotes; objects/arrays must be valid JSON.</DialogDescription>
          </DialogHeader>
          <div className="mt-2 space-y-2">
            <div className="flex flex-wrap gap-2">
              <Button variant="outline" size="sm" onClick={() => {
                try { const v = JSON.parse(editDlg.text); setEditDlg((d) => ({ ...d, text: JSON.stringify(v, null, 2) })); setEditValid({ ok: true }); } catch (e) { setEditValid({ ok: false, error: (e as Error).message }); }
              }}>Format</Button>
              <Button variant="outline" size="sm" onClick={() => {
                try { const v = JSON.parse(editDlg.text); setEditDlg((d) => ({ ...d, text: JSON.stringify(v) })); setEditValid({ ok: true }); } catch (e) { setEditValid({ ok: false, error: (e as Error).message }); }
              }}>Minify</Button>
              <Button variant="outline" size="sm" onClick={() => setEditDlg((d) => ({ ...d, text: JSON.stringify(d.text) }))}>Quote</Button>
              <Button variant="outline" size="sm" onClick={() => {
                try { const v = JSON.parse(editDlg.text); if (typeof v === 'string') setEditDlg((d) => ({ ...d, text: v })); } catch {}
              }}>Unquote</Button>
            </div>
            <Textarea value={editDlg.text} onChange={(e) => {
              const t = e.target.value; setEditDlg((d) => ({ ...d, text: t }));
              try { JSON.parse(t); setEditValid({ ok: true }); } catch (er) { setEditValid({ ok: false, error: (er as Error).message }); }
            }} className="min-h-32" />
            <div className="text-xs">
              {editValid.ok ? <span className="text-emerald-400">Valid JSON</span> : <span className="text-red-400">{editValid.error}</span>}
            </div>
          </div>
          <DialogFooter>
            <Button variant="secondary" onClick={() => setEditDlg({ open: false, node: undefined, text: "" })}>Cancel</Button>
            <Button onClick={applyEditValue} disabled={!editValid.ok}>Save</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>

      {/* Edit Key Dialog */}
      <Dialog open={editKeyDlg.open} onOpenChange={(open) => setEditKeyDlg((d) => ({ ...d, open }))}>
        <DialogContent>
          <DialogHeader>
          ...
          <div className="mt-2 space-y-2">
            <Input placeholder="newKey" value={editKeyDlg.keyText} onChange={(e) => setEditKeyDlg((d) => ({ ...d, keyText: e.target.value }))} />
          </div>
          <DialogFooter>
            <Button variant="secondary" onClick={() => setEditKeyDlg({ open: false, node: undefined, keyText: "" })}>Cancel</Button>
      ...
      {/* Add Child Dialog */}
      <Dialog open={addChildDlg.open} onOpenChange={(open) => setAddChildDlg((d) => ({ ...d, open }))}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Add child</DialogTitle>
            <DialogDescription>Append a value to an array or add a key/value to an object. Live JSON validation for value.</DialogDescription>
          </DialogHeader>
          <div className="mt-2 space-y-2">
            {(() => {
              const node = addChildDlg.node; const v = node && data ? getAtPath(data, node.path) : undefined; const isObj = v && typeof v === 'object' && !Array.isArray(v);
              return isObj ? (
                <Input placeholder="key" value={addChildDlg.keyText} onChange={(e) => setAddChildDlg((d) => ({ ...d, keyText: e.target.value }))} />
              ) : null;
            })()}
            <div className="flex flex-wrap gap-2">
              <Button variant="outline" size="sm" onClick={() => {
                try { const v = JSON.parse(addChildDlg.valueText); setAddChildDlg((d) => ({ ...d, valueText: JSON.stringify(v, null, 2) })); setAddValid({ ok: true }); } catch (e) { setAddValid({ ok: false, error: (e as Error).message }); }
              }}>Format</Button>
              <Button variant="outline" size="sm" onClick={() => {
                try { const v = JSON.parse(addChildDlg.valueText); setAddChildDlg((d) => ({ ...d, valueText: JSON.stringify(v) })); setAddValid({ ok: true }); } catch (e) { setAddValid({ ok: false, error: (e as Error).message }); }
              }}>Minify</Button>
              <Button variant="outline" size="sm" onClick={() => setAddChildDlg((d) => ({ ...d, valueText: JSON.stringify(d.valueText) }))}>Quote</Button>
              <Button variant="outline" size="sm" onClick={() => { try { const v = JSON.parse(addChildDlg.valueText); if (typeof v === 'string') setAddChildDlg((d) => ({ ...d, valueText: v })); } catch {} }}>Unquote</Button>
            </div>
            <Textarea value={addChildDlg.valueText} onChange={(e) => {
              const t = e.target.value; setAddChildDlg((d) => ({ ...d, valueText: t }));
              try { JSON.parse(t); setAddValid({ ok: true }); } catch (er) { setAddValid({ ok: false, error: (er as Error).message }); }
            }} className="min-h-32" />
            <div className="text-xs">
              {addValid.ok ? <span className="text-emerald-400">Valid JSON</span> : <span className="text-red-400">{addValid.error}</span>}
            </div>
          </div>
          <DialogFooter>
            <Button variant="secondary" onClick={() => setAddChildDlg({ open: false, node: undefined, keyText: "", valueText: "" })}>Cancel</Button>
            <Button onClick={applyAddChild} disabled={!addValid.ok}>Add</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>

      {/* Delete confirmation */}
      <AlertDialog open={deleteDlg.open} onOpenChange={(open) => setDeleteDlg((d) => ({ ...d, open }))}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Delete node?</AlertDialogTitle>
            <AlertDialogDescription>This removes the node and its children from the JSON.</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
